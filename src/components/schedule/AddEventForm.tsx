...
import { CalendarEvent, EventCategory } from '../../services/calendar/calendarData'; // Adjust path as needed
...
// --- Constants ---
const availableCategories: EventCategory[] = ['Lịch học', 'Kiểm tra', 'Kỳ thi', 'Sự kiện'];
const availableSubjects = ['Toán', 'Văn', 'Lý', 'Hóa', 'Sinh', 'Sử', 'Địa', 'Anh']; // Example subjects
const reminderOptions = ['Không', 'Khi bắt đầu', '5 phút trước', '15 phút trước', '30 phút trước', '1 giờ trước', '1 ngày trước', '2 ngày trước'];
...
const AddEventForm: React.FC<AddEventFormProps> = ({
    ...
    const handleSaveClick = () => {
        ...
        try {
            if (startTime) { // Only process if startTime is not empty
                const [startHour, startMinute] = startTime.split(':').map(Number);
                if (!isNaN(startHour) && !isNaN(startMinute)) {
                    startDateWithTime = setMilliseconds(setSeconds(setMinutes(setHours(selectedDate, startHour), startMinute), 0), 0);
                    ...
                    if (endTime) {
                        ...
                        {
                           endDateWithTime = setMilliseconds(setSeconds(setMinutes(setHours(selectedDate, endHour), endMinute), 0), 0);
                        } else if (!isNaN(endHour) && !isNaN(endMinute)) {
                           console.warn("End time is not after start time. Event might end at start time or have default duration depending on backend/display logic.");
  ...
  function getEventColor(cat: string) {
    ...
    return (
        ...
        <Box sx={{ display: 'flex', flexDirection: 'column', height: '100%', p: 2.5, overflowY: 'auto' }}>
            ...
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, mb: 1, flexShrink: 0 }}>
                ...
                <Box>
                    ...
                    <Typography variant="caption" color="text.secondary">
                     ...
                     <Typography variant="h6" sx={{ fontWeight: 'bold', lineHeight: 1.2 }}>Nhập tên sự kiện</Typography>
                </Box>
            </Box>
            ...
            {/* Event Title */}
             <TextField
                ...
                sx={{
                    mb: 2.5,
                    '& .MuiOutlinedInput-root': { borderRadius: inputBorderRadius, bgcolor: inputBackground },
                    '& .MuiOutlinedInput-input': { padding: '10px 14px', fontWeight: 500 }, // Make input text bold
                }}
                autoFocus // Focus this field when form appears
            />

            {/* Category Selection */}
            <Typography variant="caption" display="block" color="text.secondary" sx={{ mb: 1, fontWeight: 500 }}>Phân loại lịch:</Typography>
            <ToggleButtonGroup
                value={category}
                exclusive
                ...
                size="small"
                sx={{ mb: 2.5, display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(80px, 1fr))', gap: 1 }}
            >
                {availableCategories.map((cat) => (
                     <ToggleButton
                        ...
                        sx={{
                            ...
                            border: `1px solid ${alpha(theme.palette.grey[500], 0.3)} !important`, // Slightly darker border
                            textTransform: 'none',
             ...
             <Grid container spacing={1.5} sx={{ mb: 2.5 }}>
                <Grid size={{xs: 12, sm: 4}}>
                     ...
                     <FormControl fullWidth size="small">
                        <InputLabel id="subject-label" sx={{ fontSize: '0.9rem' }}>Chọn môn</InputLabel>
                        <Select
                            labelId="subject-label"
                            label="Chọn môn"
                            value={subject}
                            onChange={handleSubjectChange}
                            sx={{ borderRadius: inputBorderRadius, backgroundColor: inputBackground, fontSize: '0.9rem' }}
                        >
                            <MenuItem value="" sx={{fontSize: '0.9rem'}}><em>Không chọn</em></MenuItem>
                            {availableSubjects.map(sub => (<MenuItem key={sub} value={sub} sx={{fontSize: '0.9rem'}}>{sub}</MenuItem>))}
                        </Select>
                    </FormControl>
                </Grid>
                <Grid size={{xs: 6, sm: 4}}>
                     ...
                     <FormControl fullWidth size="small">
                         ...
                         <Typography variant="caption" component="label" htmlFor="start-time" sx={{ mb: 0.5, display: 'block', color: 'text.secondary', fontWeight: 500 }}>TG bắt đầu</Typography>
                         <TextField
                            id="start-time"
                            type="time"
                            value={startTime}
                 ...
                 <Grid size={{xs: 6, sm: 4}}>
                     {/* End Time */}
                      <FormControl fullWidth size="small">
                         <Typography variant="caption" component="label" htmlFor="end-time" sx={{ mb: 0.5, display: 'block', color: 'text.secondary', fontWeight: 500 }}>TG kết thúc</Typography>
                         <TextField
                            id="end-time"
                            type="time"
                            value={endTime}
            ...
            <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 1 }}>
                <Typography variant="body2" sx={{ fontWeight: 'medium' }}>Nhắc nhở</Typography>
                <Switch
             ...
             <FormControl fullWidth size="small" sx={{ mb: 2.5 }} disabled={!isReminderEnabled}>
                 <InputLabel id="reminder-time-label" sx={{ fontSize: '0.9rem' }}>Thông báo trước thời gian bắt đầu</InputLabel>
                <Select
                    ...
                    sx={{ borderRadius: inputBorderRadius, backgroundColor: inputBackground, fontSize: '0.9rem' }}
                >
                    {reminderOptions.map(opt => (<MenuItem key={opt} value={opt} sx={{fontSize: '0.9rem'}}>{opt}</MenuItem>))}
                </Select>
            </FormControl>
            ...
            <TextField
                ...
                sx={{
                    mb: 2.5,
                     '& .MuiOutlinedInput-root': { borderRadius: inputBorderRadius, bgcolor: inputBackground, padding: '8px 12px' }, // Adjust padding

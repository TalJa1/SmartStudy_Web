...
import { mockCalendarEvents, CalendarEvent, EventCategory } from '../../services/calendar/calendarData'; // Adjust path

// --- Event Colors Helper --- (Same as before)
const getEventColor = (category: EventCategory): { background: string; text: string; border: string } => {
    switch (category) {
        case 'Lịch học': return { background: '#E0F2E9', text: '#4CAF50', border: '#A5D6A7' };
        case 'Kiểm tra': return { background: '#FFEBEE', text: '#D32F2F', border: '#EF9A9A' };
        case 'Kỳ thi': return { background: '#E3F2FD', text: '#1976D2', border: '#90CAF9' };
        case 'Sự kiện': return { background: '#FFF3E0', text: '#EF6C00', border: '#FFCC80' };
        default: return { background: '#F5F5F5', text: '#616161', border: '#E0E0E0' };
    }
};
...
const ManualCalendarGrid = () => {
  ...
  const handleMonthMenuOpen = (event: React.MouseEvent<HTMLElement>) => setMonthAnchorEl(event.currentTarget);
  ...
  const handleDayClick = (day: Date) => {
      ...
      // Add logic to open modal or select day
  };
  ...
  // --- Render Logic ---
  const dayLabels = ['Thứ 2', 'Thứ 3', 'Thứ 4', 'Thứ 5', 'Thứ 6', 'Thứ 7', 'Chủ nhật']; // Monday first

  return (
    <Box sx={{ p: {xs: 1, sm: 2, md: 3}, display: 'flex', flexDirection: 'column', height: '100%' }}>

       {/* Top Bar: Title, View Tabs, Add Button */}
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2, flexShrink: 0 }}>
         {/* ... (Copy Toolbar from previous FullCalendar example) ... */}
         <Box sx={{ display: 'flex', alignItems: 'center' }}>
          <Typography variant="h5" sx={{ fontWeight: 'bold', mr: 3, display: { xs: 'none', sm: 'block' } }}>
            Lịch học
          </Typography>
          <Tabs value={currentView} onChange={handleViewChange} aria-label="calendar view tabs">
            <Tab label="Tháng" value="month" sx={{ textTransform: 'none', fontSize: '0.9rem', minWidth: 80 }} />
            <Tab label="Tuần" value="week" sx={{ textTransform: 'none', fontSize: '0.9rem', minWidth: 80 }} disabled />
            <Tab label="Ngày" value="day" sx={{ textTransform: 'none', fontSize: '0.9rem', minWidth: 80 }} disabled />
          </Tabs>
        </Box>
        <Button
          variant="contained"
          startIcon={<AddIcon />}
          size="small"
          sx={{ backgroundColor: '#1C2A3A', '&:hover': { backgroundColor: '#334257' }, textTransform: 'none', borderRadius: '8px' }}
      ...
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2, flexShrink: 0 }}>
        ...
        <Button
            ...
            sx={{ textTransform: 'none', color: 'text.primary', fontWeight: 'bold', fontSize: '1rem', p: 0.5 }}
        >
         ...
         >
           <MuiMenuItem onClick={() => handleNavigate('prev')}>Tháng trước</MuiMenuItem>
           <MuiMenuItem onClick={() => handleNavigate('next')}>Tháng sau</MuiMenuItem>
           <MuiMenuItem onClick={() => handleNavigate('today')}>Hôm nay</MuiMenuItem>
         </Menu>

        <Box sx={{ display: { xs: 'none', md: 'flex' }, alignItems: 'center', gap: 1 }}> {/* Hide filters on small screens */}
          <Chip label="Lịch học" size="small" variant="outlined" clickable sx={{ borderColor: '#A5D6A7', color: '#4CAF50' }} />
          <Chip label="Kiểm tra" size="small" variant="outlined" clickable sx={{ borderColor: '#EF9A9A', color: '#D32F2F' }} />
          <Chip label="Kỳ thi" size="small" variant="outlined" clickable sx={{ borderColor: '#90CAF9', color: '#1976D2' }} />
          <Chip label="Sự kiện" size="small" variant="outlined" clickable sx={{ borderColor: '#FFCC80', color: '#EF6C00' }}/>
          <IconButton size="small" sx={{ border: '1px solid', borderColor: 'divider', borderRadius: '8px', ml: 1 }}>
      ...
      <Box sx={{ flexGrow: 1, overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
        ...
        >
          {dayLabels.map((label) => (
            <Typography key={label} variant="caption" sx={{ fontWeight: 'medium', p: 1 }}>
              {label}
            </Typography>
          ))}
        </Box>
            ...
            return (
              <Box
                ...
                sx={{
                  ...
                  p: '4px',
                  cursor: 'pointer',
                  backgroundColor: isCurrentDay ? alpha(theme.palette.primary.light, 0.1) : 'inherit',
              ...
              >
                ...
                {/* Events */}
                <Box sx={{ mt: '20px', display: 'flex', flexDirection: 'column', gap: '2px' }}>
                  {dayEvents.map((event) => {
                     const colors = getEventColor(event.category);
                     return (
                       <Box
                         key={event.id}
                         onClick={(e) => { e.stopPropagation(); handleEventClick(event); }} // Prevent day click
                         sx={{
                       ...
                       >
                         ...
                         {event.description && <Typography variant="caption" component="div" sx={{ fontStyle: 'italic', opacity: 0.9, lineHeight: 1 }}>{event.description}</Typography>}
